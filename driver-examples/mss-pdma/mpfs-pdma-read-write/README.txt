...
over the UART terminal indicating the  DMA channels for transaction. 
...
mpfs-pdma-read-write renode all-harts debug.launch which is configured for Renode.
Alternatively, you can perform above two steps in one go by launching 
mpfs-pdma-read-write renode all-harts start-platform-and-debug.launch which
...
Build the project and launch the debug configuration named 
mpfs-pdma-read-write hw all-harts debug.launch which is configured for the 
