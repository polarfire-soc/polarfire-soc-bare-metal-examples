 ...
 *
 * CoreMMC Register Description File
 *
 * SVN $Revision$
 * SVN $Date$
 */
...
/*------------------------------------------------------------------------------
 * Major Version Register details - Only present if Hardware Revision Number in
 * VR_REG_OFFSET register == 0x03
 * Uses all 8 bits
 */
...
/*
 * SUER bits. User Error. FIFO has not been filled.
 */
#define IMR_MUER_OFFSET   0x2Cu
#define IMR_MUER_MASK     0x80u
#define IMR_MUER_SHIFT    7u

#define IMR_MASK_MULTI_READ (0xFF & ~( IMR_MCSI_MASK /*| IMR_MUER_MASK*/))  /*Command send, user error masked */
#define IMR_MASK_MULTI_WRITE (0xFF & ~( IMR_MCSI_MASK /*| IMR_MUER_MASK*/)) /*Command send, user error masked */
...
#define BIMR_MASK_MULTI_READ      (0xFF & ~0x03) /* all except block read and 
                                                   block write done */
#define BIMR_MASK_MULTI_WRITE     (0xFF & ~0x03) /* all except block read and 
                                                   block write done */
...
#define MBIMR_MASK_MULTI_READ      (0xFF & ~MBISR_MBWDONE_MASK) /* all except multi-block write done     */
#define MBIMR_MASK_MULTI_WRITE     (0xFF & ~MBIMR_MBRDONE_MASK) /* all except multi-block read done     */
...
#define ISR_ERRORS  (ISR_SUER_MASK | ISR_SSBI_MASK | ISR_STBI_MASK |\
                     ISR_STXI_MASK | ISR_SBOI_MASK | ISR_SBUI_MASK )
...
/*
 * SDATTO bits. Block Read start Time-out.
 */
...
#define BISR_ERRORS  (BISR_SWBFIFOTOERR_MASK | BISR_SWBUSYTOERR_MASK | \
                      BISR_SCRCSTERR_MASK | BISR_SBRSTERR_MASK | \
                      BISR_SBRTOUTERR_MASK | BISR_SBCRCERR_MASK)
...
/*
 * MDATTO bits. Block Read Data Time-out
 */
...
/*
 * MCRCSTERR bits. Block CRC Response Error occurred with stop bit error
 */
...
/*
 * MWBUSYTOERR bits. Card not ready-- timeout Data0 down for too long eMMC 
 * slave doesn't hold DAT0 low indefinitely in single & multiple block 
 * transfers start bit to be loaded onto the bus by the eMMC slave from either 
 * the SBRSTRT or the MBRSTRT bit being set in the SBCSR or the MBSCR.
 */
 ...
 * write transfer Or from the stop bit of the CRC status frame to there being 
 * a block length amount of data in the write FIFO for subsequent blocks in a 
...
#define MBISR_ERRORS  (MBISR_MWBFIFOTOERR_MASK | MBISR_MWBUSYTOERR_MASK | \
                       MBISR_MCRCSTERR_MASK | MBISR_MBRSTERR_MASK | \
                       MBISR_MBRTOUTERR_MASK | MBISR_MBCRCERR_MASK)
#define ISR_BISR_MBSIR_ERR ((MBISR_ERRORS<<16) | (BISR_ERRORS<<8) | ISR_ERRORS)
