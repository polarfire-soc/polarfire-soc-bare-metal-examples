...
typedef enum R1_CARD_STATUS_RESPONSE_
{
    ...
    CURRENT_STATE_RCV        = ((0x06u<<1)<<16),    /* in this state when rx data from eMMC */
    CURRENT_STATE_DATA       = ((0x05u<<1)<<16),    /* in this state when sending data to eMMC */
...
#define R1_ERR_RESP_MASK      (SWITCH_ERROR | WP_ERASE_SKIP | ERASE_RESET | \
                               COM_CRC_ERROR | ILLEGAL_COMMAND | \
                               CARD_ECC_FAILED | CC_ERROR | GEN_ERROR | \
                               UNDERRUN | OVERRUN | CSD_OVERWRITE| \
                               ADDRESS_OUT_OF_RANGE | ADDRESS_MISALIGN | \
                               BLOCK_LEN_ERROR | ERASE_SEQ_ERROR | ERASE_PARAM | \
                               WP_VIOLATION | LOCK_UNLOCK_FAILED) 
