 ...
 *
 * CoreMMC Interface Level Driver.
 * 
 * This Core MMC Interface driver provides functions for transferring 
 * configuration and programming commands to the eMMC device. Functions 
 * contained within the Core MMC Interface Driver are accessed through the 
...
)
{
    ...
    switch (cmd_option)
    {
        case CHECK_IF_CMD_SENT_POLL: /* only need to wait around if expecting no response */
 ...
 */
static cif_response_t response_1_parser
(
    addr_t addr
)
{
