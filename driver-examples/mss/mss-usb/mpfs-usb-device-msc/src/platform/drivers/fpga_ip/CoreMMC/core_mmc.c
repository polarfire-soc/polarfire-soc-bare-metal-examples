 ...
 *
 * CoreMMC bare metal driver implementation for CoreMMC.
 *
 * This Core MMC driver provides functions for implementing eMMC master
 * operations such as block read and block write transfers.
 *
 * SVN $Revision$
 * SVN $Date$
 */
...
)
{
    ...
    if(MMC_TRANSFER_SUCCESS == ret_status)
    {
         ...
         */
        response_status = cif_send_cmd(this_mmc->base_addr,
                                       RESET_ARG,
        ...
        if(TRANSFER_IF_SUCCESS == response_status)
        {
            ...
            if(MMC_TRANSFER_SUCCESS == ret_status)
            {
                ...
                this_mmc->access_mode = ((SECTOR_ACCESS_MODE_MASK &
                                          response_reg) >> SECTOR_SHIFT);
                ...
                if(MMC_TRANSFER_SUCCESS == ret_status)
                {
                    ...
                    {
                        ret_status = MMC_single_block_read(this_mmc, READ_SEND_EXT_CSD, csd_reg);
                        if(MMC_TRANSFER_SUCCESS == ret_status)
                        {
...
)
{
    ...
    do
    {
        blk_write = HAL_get_8bit_reg_field(this_mmc->base_addr,
                                           BISR_SBWDONE);
    } while(DONE != blk_write);
    blk_tran_err = (NO_BLOCK_TRAN_ERR &
                    HAL_get_8bit_reg(this_mmc->base_addr, BISR));
 ...
 * See ".h" for details of how to use this function.
 * Before entering this function, MMC_init_fifo must be called, and mechanism set-up
 * to fill FIFO.
 */
...
)
{
    ...
    this_mmc->status.int_state = MMC_ISR_SW_PARSE_CMD13;   /* Set the current CoreMMC interrupt state machine state */
    ...
    send_mmc_cmd(this_mmc->base_addr, RCA_VALUE, MMC_CMD_13_SEND_STATUS, CHECK_IF_CMD_SENT_NO);
 ...
 )
{
    ...
    /* Initialize interrupt state machine */
    this_mmc->status.int_state = MMC_ISR_MW_PARSE_CMD13;   /* Set the current CoreMMC interrupt state machine state */
    this_mmc->status.no_of_blks = no_of_blks;              /* Number of blocks to either send or received in current command */
    this_mmc->status.blk_addr = dst_addr;                  /* Block to start read/write from */
    this_mmc->status.start_systick = current_timeout_ticks;/* Set at start- to enable timeout */
    this_mmc->status.timeout_value_ticks = timeout_ticks;  /* In systicks */
    this_mmc->status.err_state = MMC_CMD_PROCESSING;       /* Error report from interrupt */
...
)
{
    ...
    if(!(ISR_BISR_MBSIR_ERR & isr_errors)) /* Abort if any errors*/
    {
        ...
        read_err_isr = ISR_READ_ERR & HAL_get_8bit_reg(this_mmc->base_addr,
                                                       ISR);
...
)
{
    ...
    /* Enable required interrupts in CoreMMC- for sending commands / error checking */
 ...
 * State.
 */
static cif_response_t set_mmc_dwidth
(
    const mmc_instance_t * this_mmc
)
{
    ...
    response_status = cif_send_cmd(this_mmc->base_addr,
                          MMC_DW_CSD | ((uint32_t)this_mmc->hw_mmc_dwidth << 8u),
                          MMC_CMD_6_SWITCH);
    /*
 ...
 */
static mmc_transfer_status_t get_hardware_config
(
    mmc_instance_t * this_mmc
)
{
    ...
    this_mmc->hw_fifo_depth = HAL_get_8bit_reg_field(this_mmc->base_addr,
                                                     VR_MMCFD);
    this_mmc->hw_mmc_dwidth = HAL_get_8bit_reg_field(this_mmc->base_addr,
                                                     VR_MMCDW);
    this_mmc->hw_maj_revision = HAL_get_8bit_reg_field(this_mmc->base_addr,
            VR_REV);
    
    /* If revision is equal to 3, indicates Major revision now stored in VR_MAJ_REG
     * and minor in VR_MIN_REG */
    if(this_mmc->hw_maj_revision == 0x03U)
    {
        this_mmc->hw_maj_revision = HAL_get_8bit_reg(this_mmc->base_addr,
                                                     VR_MAJ);
        this_mmc->hw_min_revision = HAL_get_8bit_reg(this_mmc->base_addr,
                                                     VR_MIN);
    }
    else
    {
 ...
 * Standby State.
 */
static void get_card_sector_size
(
    mmc_instance_t * this_mmc
)
{
    ...
    csd_max_sector_lwr = SECT_SIZE_LWR_CSD_MASK &
                  ((uint32_t)HAL_get_8bit_reg(this_mmc->base_addr, RR13) >> 6u);
    
    csd_max_sector_upr = SECT_SIZE_UPR_CSD_MASK &
 ...
 * Transfer State.
 */
static cif_response_t set_mmc_clk_div
(
    const mmc_instance_t * this_mmc
)
{
 ...
 */
static mmc_transfer_status_t oper_cond_config
(
    const mmc_instance_t * this_mmc
)
{
 ...
 */
static mmc_transfer_status_t data_transfer_config
(
    mmc_instance_t * this_mmc
)
{
 ...
 * to 0x0001.
 */
static mmc_transfer_status_t dev_addr_config
(
    const mmc_instance_t * this_mmc
)
{
    ...
    response_status = cif_send_cmd(this_mmc->base_addr, STUFF_BITS,
    ...
    else
    {
        switch(this_mmc->status.int_state)
        {
            ...
            case MMC_ISR_MW_PARSE_CMD23:
                ...
                send_mmc_cmd(this_mmc->base_addr, this_mmc->status.blk_addr, MMC_CMD_25_WRITE_MULTI_BLOCK, 
                             CHECK_IF_CMD_SENT_NO);
            ...
            case MMC_ISR_MW_PARSE_CMD25:
                ...
                /* No need to check to see if busy as Core MMC will handle this situation when sending */
                /* tell Core MMC to kick-off write */
                ...
                break;
            case MMC_ISR_MW_END:
                /* Check Core MMC multi-write bit finished*/
            ...
            case MMC_ISR_SW_PARSE_CMD13:
                 ...
                 * to the MMC device and wait until the BDONE signal is set.
                 */
                send_mmc_cmd(this_mmc->base_addr, this_mmc->status.blk_addr, MMC_CMD_24_WRITE_SINGLE_BLOCK,
                             CHECK_IF_CMD_SENT_NO);
            ...
            case MMC_ISR_SW_PARSE_CMD24:
                ...
                break;
            case MMC_ISR_SW_END:
                /* Check Core MMC single-write bit finished*/
            ...
            case MMC_ISR_MR_PARSE_CMD23:
                ...
                /* Set-up Core MMC to expect read */
                ...
                send_mmc_cmd(this_mmc->base_addr, this_mmc->status.blk_addr, MMC_CMD_18_READ_MULTIPLE_BLOCK, CHECK_IF_CMD_SENT_NO);
            ...
            case MMC_ISR_MR_PARSE_CMD18:
                ...
                /* No need to check to see if busy as Core MMC will handle this situation when sending or receiving multi
                 * blocks no need to do anything else, read will be signaled via DMA, or next state if manually 
                 * transferring. */
                this_mmc->status.int_state = MMC_ISR_MR_END;
                break;
            case MMC_ISR_MR_END:
                /* Check Core MMC multi-read bit finished*/
            ...
            case MMC_ISR_EXT_CSD_PARSE_CMD13:
                ...
                {
                    send_mmc_cmd(this_mmc->base_addr, this_mmc->status.blk_addr, MMC_CMD_17_READ_SINGLE_BLOCK, 
                                 CHECK_IF_CMD_SENT_NO);
                }
                else
                {
                    send_mmc_cmd(this_mmc->base_addr, 0, MMC_CMD_8_SEND_EXT_CSD, CHECK_IF_CMD_SENT_NO);
                }
            ...
            case MMC_ISR_ERROR:     /* An easy way for driver to know if something went wrong */
                /* Clear masks */
                HAL_set_32bit_reg(this_mmc->base_addr, IMR, 0x00000000U);
                this_mmc->status.int_state = MMC_ISR_ERROR;
                break;
            case MMC_ISR_IDLE:      /* Should never get an interrupt here, if we do, go to error state */
                /* Clear masks */
                HAL_set_32bit_reg(this_mmc->base_addr, IMR, 0x00000000U);
                this_mmc->status.int_state = MMC_ISR_ERROR;
                break;
            ...
            case MMC_ISR_STOP_TRANSMISSION:
                ...
                {
                    send_mmc_cmd(this_mmc->base_addr, RCA_VALUE, MMC_CMD_12_STOP_TRANSMISSION, CHECK_IF_CMD_SENT_NO);
                    break;
                }
                else
                {
