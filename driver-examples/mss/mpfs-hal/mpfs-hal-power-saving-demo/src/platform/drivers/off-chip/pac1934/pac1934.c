...
void PAC1934_drawISense(void)
{
    ...
    MSS_UART_polled_tx(gp_usr_uart, (const uint8_t *)g_ui_buf, UIBufSize);
    MSS_UART_polled_tx_string(gp_usr_uart, (const uint8_t *)lineSymbol);
    MSS_UART_polled_tx_string(gp_usr_uart, (const uint8_t *)"\n\r\n\rType 0 to show the menu again\n\r");
    MSS_UART_polled_tx_string(gp_usr_uart, (const uint8_t *)uart_page_break);
