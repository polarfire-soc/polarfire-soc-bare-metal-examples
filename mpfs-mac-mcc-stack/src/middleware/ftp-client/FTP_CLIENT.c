...
#include "mcc_tcpip_lite/tcpv4.h"
 ...
 */
uint32_t ftpc_readln_get(uint8_t *buffer, uint16_t requested);
uint32_t ftpc_readln_get(uint8_t *buffer, uint16_t requested)
{
    uint32_t ret_count = 0U;
    uint32_t retval;
...
{
    ftp.message = str;
    ftp.msglen = (uint32_t)strlen(ftp.message);

    if(NET_SUCCESS == TCP_SendDone(&ftp.s)) /* Nothing outstanding at the moment... */
    {
        if(NET_SUCCESS == TCP_Send(&ftp.s, (uint8_t *)ftp.message, ftp.msglen))
        {
            FTP_SETSTATE2(FTP_MSGWAIT, nextstate);
            ftp.msgcomplete = ftp.msglen; /* Indicate we have the send in the pipeline */
        }
        else
        {
#ifdef FTP_VERBOSE
            printf("FTPC: ftpc_msg() - TCP_Send() failure\n");
#endif
            FTP_SETSTATE(FTP_BAIL);
        }
    }
    else /* Wait for previous to complete and then send */
    {
        FTP_SETSTATE2(FTP_MSGWAIT, nextstate);
        ftp.msgcomplete = 0; /* Indicate we don't have the send in the pipeline */
    }
...
void ftpc_finish_msg(void)
{
    ...
    socket_state = TCP_SocketPoll(&ftp.s);
    if(SOCKET_CLOSED == socket_state) /* socket was probably closed immediately after QUIT? */
    {
        FTP_SETSTATE(FTP_DONE2);
    }
    else if(0 == ftp.msgcomplete) /* Need to test for last tx done and send message */
    {
        if(NET_SUCCESS == TCP_SendDone(&ftp.s)) /* Nothing outstanding at the moment... */
        {
            if(NET_SUCCESS == TCP_Send(&ftp.s, (uint8_t *)ftp.message, (size_t)ftp.msglen))
            {
                ftp.msgcomplete = ftp.msglen; /* Indicate we have the send in the pipeline */
            }
            else
            {
    #ifdef FTP_VERBOSE
                printf("FTPC: ftpc_finish_msg() - TCP_Send() failure\n");
    #endif
                FTP_SETSTATE(FTP_BAIL);
            }
        }
    ...
    {
        if(NET_SUCCESS == TCP_SendDone(&ftp.s)) /* Nothing outstanding at the moment so move on... */
        {
    ...
    const char  *dir, char *buffer, int length)
{
    ...
    TCP_SocketInit(&ftp.s); /* Todo: PMCS check return */
    ftp.s_in_use = true;

//    if(ftp.mode & FTP_MODE_PASSIVE) /* Passive mode only uses data socket */
//    {
        TCP_SocketInit(&ftp.dtp_s); /* Todo: PMCS check return */
//        TCP_SockRemove(&ftp.lstn_s);
        ftp.dtp_s_in_use = true;
//    }
//    else /* Active mode needs listening socket and data socket which is initialised later on... */
//    {
//        TCP_SockInit(&ftp.lstn_s); /* Todo: PMCS check return */
//        TCP_SockRemove(&ftp.dtp_s);
//        ftp.lstn_s_in_use = true;
//        fcntl(ftp.lstn_s, F_SETFL, O_NONBLOCK); /* Must be non blocking or the accept() will block */
//    }


    if((ftp.s_in_use && (NOT_A_SOCKET == TCP_SocketPoll(&ftp.s))) ||
       (ftp.dtp_s_in_use && (NOT_A_SOCKET == TCP_SocketPoll(&ftp.dtp_s)))) // ||
//       (ftp.lstn_s_in_use && (NOT_A_SOCKET == TCP_SocketPoll(&ftp.lstn_s))))
    {
#ifdef FTP_VERBOSE
        printf("FTPC: Cannot create sockets\n");
        //sock_perror(&ftp.sock, NULL);
#endif
        if(SOCKET_CLOSED == TCP_SocketPoll(&ftp.s))
        {
            TCP_SocketRemove(&ftp.s); /* Remove is ok here as we have not really started anything yet */
        }

        if(SOCKET_CLOSED == TCP_SocketPoll(&ftp.dtp_s))
        {
            TCP_SocketRemove(&ftp.dtp_s); /* Remove is ok here as we have not really started anything yet */
        }

//        if(NET_SUCCESS == TCP_SocketPoll(&ftp.lstn_s))
//        {
//            TCP_SocketRemove(&ftp.lstn_s); /* Remove is ok here as we have not really started anything yet */
//        }
...
//    ftp_init_done = 1;
    ...
    host_addr.port        = htons((uint16_t)port);
    host_addr.addr.s_addr = htonl(host);

    TCP_InsertRxBuffer(&ftp.s, ftp.s_rx_buf1 + 1, sizeof(ftp.s_rx_buf1));
    ftp.s_use_buf1 = true;
    response = TCP_Connect(&ftp.s, &host_addr);

    //response = connect(ftp.s, (struct sockaddr *)&host_addr, sizeof(host_addr));
    if(NET_SUCCESS != response)
    {
#ifdef FTP_VERBOSE
...
void ftpc_dtp_open(void)
{
    ...
    if (!(ftp.mode & FTP_MODE_PASSIVE)) // Not passive mode so we pick a port
    {
         ...
         */
        if(0 != ip_database_info.ipv4_myAddress)
        {
            ftp.dtpaddr = htonl(ip_database_info.ipv4_myAddress);
        }
        else
        {
#ifdef FTP_VERBOSE
            printf("FTPC: IP address not currently set\n");
#endif
            FTP_SETSTATE(FTP_BAIL);
            return;
        }
        ...
        response = TCP_Bind(&ftp.dtp_s, 0); /* We will need a new port... */
        response = TCP_Listen(&ftp.dtp_s);
        if(NET_SUCCESS != response)
        {
#ifdef FTP_VERBOSE
        ...
        {
            ftp.dtpport = ftp.dtp_s.localPort;
        }
#ifdef FTP_CLIENT_KEEPALIVE_WAITTIME
//        tcp_keepalive(&ftp.sock, FTP_CLIENT_KEEPALIVE_WAITTIME);
#endif
...
int ftp_client_tick(void)
{
    ...
    if((SOCKET_CONNECTED != TCP_SocketPoll(&ftp.s)) && (FTP_STATE != FTP_DONE3) && (FTP_STATE != FTP_DONE2) && (0 == got_data_file))
    {
#ifdef FTP_VERBOSE
    ...
    case FTP_CONNECTWAIT:
        ...
        socket_state = TCP_SocketPoll(&ftp.s);
        if (SOCKET_CONNECTED == socket_state)
        {
        ...
        else
        {
            ...
            if(SOCKET_IN_PROGRESS != socket_state)
            {
#ifdef FTP_VERBOSE
    ...
    case FTP_PASV:
        ...
        his_addr.port        = htons(hisport);
        his_addr.addr.s_addr = htonl(hisip);

        response = TCP_Connect(&ftp.dtp_s, &his_addr);
...
//        tcp_keepalive(&ftp.sock, FTP_CLIENT_KEEPALIVE_WAITTIME);
    ...
    case FTP_UPLOAD:
        ...
        socket_state = TCP_SocketPoll(&ftp.s);
        if((SOCKET_CONNECTED != socket_state) && ((SOCKET_IN_PROGRESS != socket_state)))
        {
        ...
        else
        {
            ...
            printf("FTPC: FTP_UPLOAD 2 - bad data handler return %d\n", rc);
                ...
                response = TCP_SendDone(&ftp.dtp_s);
                while(NET_SUCCESS != response)
                {
                    Network_Manage(); /* Keep the network stack ticking over - need some sort of timeout... */
                    response = TCP_SendDone(&ftp.dtp_s);
                }
                ...
                if (rc > 0) /* This is how much we actually put in the buffer */
                {
                    do /* Write this chunk */
                    {
                        ...
                        response = TCP_Send(&ftp.dtp_s, ftp.line, rc);
                        response = TCP_SendDone(&ftp.dtp_s);
                        while(NET_SUCCESS != response)
                        {
                            Network_Manage(); /* Keep the network stack ticking over - need some sort of timeout... */
    ...
    case FTP_UPLOAD2:
        socket_state = TCP_SocketPoll(&ftp.dtp_s);
        if(NOT_A_SOCKET == socket_state)
        {
#ifdef FTP_VERBOSE
            printf("FTPC: FTP Data Socket borked.\n");
            printf("FTPC: Trying for FTP Code 226 to verify successful send.\n");
#endif
            FTP_SETSTATE2(FTP_GETLINE, FTP_UPLOAD3);
        }
        else if(SOCKET_CONNECTED != socket_state)
        {
#ifdef FTP_VERBOSE
    ...
    case FTP_DOWNLOAD:
        ...
        socket_state = TCP_SocketPoll(&ftp.dtp_s);
        if(SOCKET_IN_PROGRESS == socket_state) /* Still waiting... */
        {
            break;
        }

        if(SOCKET_CONNECTED != socket_state) /* Not an expected state... */
        {
#ifdef FTP_VERBOSE
            printf("FTPC: FTP Data Socket borked.\n");
#endif
        ...
        FTP_SETSTATE(FTP_DOWNLOADING); /* Fall thru */
    case FTP_DOWNLOADING:
        socket_state = TCP_SocketPoll(&ftp.dtp_s);
        if(TCP_GetRxLength(&ftp.dtp_s) <= 0 && (SOCKET_CONNECTED != socket_state))
        {
#ifdef FTP_VERBOSE
        ...
        if (!ftp.buffer) /* Data Handler mode */
        {
            ...
            if(TCP_GetRxLength(&ftp.dtp_s) > 0UL)
            {
                ret_val = TCP_GetReceivedData(&ftp.dtp_s);
                if(ftp.d_use_buf1)
                {
                    ftp.d_rx_len = ret_val;
                    TCP_InsertRxBuffer(&ftp.dtp_s, ftp.d_rx_buf2, sizeof(ftp.d_rx_buf2));
                    ftp.d_use_buf1 = false;
                    d_buffer = ftp.d_rx_buf1;
                }
                else
                {
                    ftp.d_rx_len = ret_val;
                    TCP_InsertRxBuffer(&ftp.dtp_s, ftp.d_rx_buf1, sizeof(ftp.d_rx_buf1));
                    ftp.d_use_buf1 = true;
                    d_buffer = ftp.d_rx_buf2;
                }

                if(ret_val > 0UL)
                {
                    rc = ftp.dhnd(d_buffer, ret_val, ftp.offset, FTPDH_IN, ftp.dhnd_data);
                    if (rc < 0)
                        goto _ftp_client_bail;
                    ftp.offset += ret_val;
                }
            ...
            if (rc) /* Buffer not full */
            {
                if(TCP_GetRxLength(&ftp.dtp_s) > 0UL)
                {
                    ...
                    if(ret_val > 0UL)
                    {
                        ...
                        memcpy(ftp.offset, d_buffer, ret_val);
                        ftp.offset += ret_val;
                    }
                }
            }
            else /* Buffer full so read and discard... */
            {
                if(TCP_GetRxLength(&ftp.dtp_s) > 0UL)
                {
                    ret_val = TCP_GetReceivedData(&ftp.dtp_s);
 ...
 */
    ...
    case FTP_DONE3:
        ...
        if((xTaskGetTickCount() - ftp.timeout) > ((FTP_TIMEOUT * 1000L) / 1000 /* portTICK_RATE_MS */))
        {
#ifdef FTP_VERBOSE
