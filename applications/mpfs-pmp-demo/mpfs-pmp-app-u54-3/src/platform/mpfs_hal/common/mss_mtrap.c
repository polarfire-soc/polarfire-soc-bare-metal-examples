...
void trap_from_machine_mode(uintptr_t * regs, uintptr_t dummy, uintptr_t mepc)
{
    ...
    else if ((mcause == CAUSE_STORE_ACCESS) | (mcause == CAUSE_LOAD_ACCESS) | (mcause == CAUSE_FETCH_ACCESS))
    {
#ifdef PMP_DEBU_EN
        char info_string[80];
        uint64_t hartid = read_csr(mhartid);
        sprintf(info_string, "\r\nInstruction address which caused the trap is 0x%lx\r\n", mepc);
        MSS_UART_polled_tx(g_uart, (const uint8_t*)info_string,(uint32_t)strlen(info_string));
        sprintf(info_string, "U54_%d access to address 0x%lx is denied \r\n",hartid, dummy );
        MSS_UART_polled_tx(g_uart, (const uint8_t*)info_string,(uint32_t)strlen(info_string));
        sprintf(info_string,"Exception#: 0x%lx \r\n", mcause );
        MSS_UART_polled_tx(g_uart, (const uint8_t*)info_string,(uint32_t)strlen(info_string));
        sprintf(info_string,"Exception cause: %s\r\n\n", exception_code[mcause]);
        MSS_UART_polled_tx(g_uart, (const uint8_t*)info_string,(uint32_t)strlen(info_string));
        sprintf(info_string,"\r\n\nU54_%d is getting reset.....            \r\n", hartid);
        MSS_UART_polled_tx(g_uart, (const uint8_t*)info_string,(uint32_t)strlen(info_string));
#endif
        {
            extern int reset_vector(void);
            write_csr(mepc, reset_vector);
        }
    }
    else
    {
