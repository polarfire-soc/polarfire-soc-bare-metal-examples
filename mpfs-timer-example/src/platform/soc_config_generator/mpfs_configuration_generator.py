...
# 
# 0.6.5 Added .cxz file support. The script will take .cxz file generated by polarfire soc mss configurator as an input and extract
#       xml configurations file from it.
#
# 0.6.4 removing redundancy by comparing the time stamp of fpga_design_config.h and latest xml_file
#       if output_file_time_stamp > xml_file time_stamp then exit . 
#  
...
def show_help():
    ...
    print \
        (' Usage: python3 mpfs_configuration_generator.py [xml file path]\
         [output folder path] ')
    print('path can be absolute as well as relative \n')
    input(' Please run again with correct arguments')

#------------------------------------------------------------------------------
# extract xml from the zipfile with .cxz extension.
#------------------------------------------------------------------------------

def extract_xml_from_zip(input_folder_path):

    ''' This function will extract xml file from the .cxz file. 
        [params] input_folder_path -: path to the folder containing.cxz or .xml file
        [return] -: None 
                    
    '''

    # check if .cxz file is present in input_folder_path 
    flag = False
    zip_file = None
    for file in os.listdir(input_folder_path):
        print(file)
        if file.endswith('.cxz') == True:
            zip_file = os.path.join(input_folder_path,file)
            flag = True
            break
    if flag == False:
        print(".cxz file is not present ")
        print(" aborting xml extraction ............")
        return

    # list of xml files present in the input_folder_path
    xml_list = [ file for file in os.listdir(input_folder_path) if file.endswith(".xml") ] 
    print(xml_list)
    ...
    with ZipFile(zip_file, 'r') as zip :
        ...
        # return if xml_file already had been extracted. 
        for xml_file in xml_list : 
            if xml_file in file_list : 
                print("xml file is already extracted ")
                return 

        # get ziped xml file name  
        ziped_xml_name = [ file for file in file_list if file.endswith(".xml") ][0] 

        # extract xml file from .cxz file
        zip.extract(ziped_xml_name,path=input_folder_path) 

        print("\n*******************************************************************")
        print("                 xml file extraction complete.                       ")
        print("*******************************************************************\n")
...
def main_config_generator():
    ...
    input_folder_path = argumentList[0]

    # extract xml file from .cxz file if present. 
    print(" extracting xml file from '.cxz' zip file ....")
    extract_xml_from_zip(input_folder_path)

    # get_full_path will return the absolute path of selected xml file based on
    # time_stamp comparision. 
    input_xml_file = get_full_path(input_folder_path)
    ...
    # If the target folder is not empty and target xml_time_stamp /
    # is less than fpaga_design_config.h then exit with the message
    # This is to avoid unnecessary build step. 
    ...
    if (os.path.isfile(output_file_path)): 
        if(os.path.getsize != 0):
            ...
            output_file_ts = os.path.getmtime(output_file_path)
            xml_file_ts = os.path.getmtime(input_xml_file)

            if (output_file_ts > xml_file_ts ): 

                print("\n*******************************************************************")
                print("  fpga_design_config is already upto date with the latest xml file  ")
                print("********************************************************************")
                print("exiting build step ..... ")
                sys.exit()
    else:
        pass 
