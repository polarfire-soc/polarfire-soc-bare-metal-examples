...
Type 6  Generate message hart 0 to hart 1\r\n\
";
...
void e51(void)
{
    ...
    uint8_t message[8];
    uint8_t data_test[IHC_MESSAGE_SIZE];
      ...
      spinunlock(&hart_share->mutex_uart0);
        ...
        if (rx_size > 0)
        {
            switch(rx_buff[0])
            {
                ...
                case '6':
                    remote_hart_id = 1U;
                    message[0] = data_test[0]++;
                    tx_message(remote_hart_id, hls->my_hart_id, (uint8_t *)&message);
                    break;
